...
const DEFAULT_TOKEN_SERVER_URL = 'http://127.0.0.1:7879';
const TOKEN_SERVER_TIMEOUT_MS = 3000;
...
let configuredTokenServerUrl = null;
...
async function fetchTokenFromServer() {
  if (configuredTokenServerUrl == null) return;
  try {
    const response = await fetch(configuredTokenServerUrl, {
      method: 'GET',
      signal: AbortSignal.timeout(TOKEN_SERVER_TIMEOUT_MS),
    });
    if (response.ok) {
      const token = (await response.text()).trim();
      if (token.length > 0 && !token.startsWith('ERROR:')) {
        configuredImpervaToken = token;
        logger.debug('Fotocasa: refreshed X-D-Token from token server');
      }
    }
  } catch {
    // Server not running or timed out — fall back to cached/static token silently.
    logger.debug('Fotocasa: token server unreachable, using cached X-D-Token');
  }
}
...
export const init = (sourceConfig, blacklist) => {
  ...
  configuredTokenServerUrl = null;

  configuredImpervaToken = resolveImpervaToken(safeSourceConfig);
  configuredCookieHeader = resolveCookieHeader(safeSourceConfig);

  // Token server: explicit config/env, or auto-try the default local URL when
  // no static token is configured (so users only need to run the setup script).
  const explicitServerUrl = stringify(
    safeSourceConfig?.xDTokenServerUrl ?? safeSourceConfig?.tokenServerUrl ?? process.env.FOTOCASA_TOKEN_SERVER_URL,
  );
  configuredTokenServerUrl = explicitServerUrl ?? (configuredImpervaToken == null ? DEFAULT_TOKEN_SERVER_URL : null);

  if (configuredImpervaToken != null || configuredCookieHeader != null || configuredTokenServerUrl != null) {
