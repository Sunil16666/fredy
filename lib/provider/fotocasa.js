...
const ENABLE_SEARCH_GW_FALLBACK = process.env.FOTOCASA_ENABLE_SEARCH_GW_FALLBACK === 'true';
const DEFAULT_BASE_URLS = ENABLE_SEARCH_GW_FALLBACK ? [API_GATEWAY_BASE_URL, SEARCH_BASE_URL] : [API_GATEWAY_BASE_URL];
...
function parseSearchUrlToQuery(searchUrl) {
  ...
  const directMappings = [
    ...
    ['propertySubTypeList', ['propertySubTypeList', 'propertySubtypeList', 'propertySubtypeIds']],
  ];
  ...
  for (const [queryKey, paramKeys] of directMappings) {
    for (const paramKey of paramKeys) {
      ...
      if (value != null && value.trim().length > 0) {
        putIfPresent(query, queryKey, decodeURIComponentSafe(value));
        break;
      }
    }
  }
